...
export class EditClassComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.classId = window.location.href.split("/")[window.location.href.split("/").length - 1];
    this.getAllSubjects();
    this.getAllTeachers();
  }

  getClass = async () => {
    const response = await fetch(`${environment.API_URL}/api/class/getClass/${this.classId}`, {
      method: 'GET',
      headers: {
    ...
    this.fee = data.class.fee;
    this.description = data.class.description
    this.addedSubjects = data.class.subjects;
  };
